...
export const useSmartDownload = () => {
  ...
  const initiateDownload = async (
    ...
    // Check user tier permissions - only Pro/Premium users can download
    if (!isPro && !isPremium) {
    ...
    try {
      ...
      // Use Direct vidsrc.vip Links as Primary Method (as required)
      if (contentId) {
        let directUrl = '';

        if (contentType === 'movie') {
          directUrl = `https://dl.vidsrc.vip/movies/${contentId}`;
        } else if (contentType === 'tv' || contentType === 'series' || contentType === 'anime') {
          if (seasonNumber !== undefined && episodeNumber !== undefined) {
            directUrl = `https://dl.vidsrc.vip/tv/${contentId}/${seasonNumber}/${episodeNumber}`;
          } else {
            directUrl = `https://dl.vidsrc.vip/tv/${contentId}/1/1`; // Default logic
          }
        }
      ...
      // Fallback to AI search only if direct URL method fails
      ...
      if (searchResult.success && (isPro || isPremium) && searchResult.nkiriUrl) {
