...
export const useWatchTracking = () => {
  ...
  const startWatchSession = useCallback(async (
    ...
    try {
      ...
      let sessionData: WatchSession;
      if (data && typeof data === 'object') {
        const d = data as Record<string, unknown>;
        const rawEvents = d.watch_events;
        let events: WatchEvent[] = [];
        if (Array.isArray(rawEvents)) {
          events = rawEvents as WatchEvent[];
        } else if (typeof rawEvents === 'string') {
          try {
            events = JSON.parse(rawEvents) as WatchEvent[];
          } catch (_) {
            events = [];
          }
        }
        ...
        sessionData = {
          ...
          content_title: d.content_title as string | undefined,
          content_duration: typeof d.content_duration === 'number' ? (d.content_duration as number) : undefined,
          session_start: (d.session_start as string) || new Date().toISOString(),
          session_end: d.session_end as string | undefined,
          total_watched_time: typeof d.total_watched_time === 'number' ? (d.total_watched_time as number) : 0,
          watch_events: events,
          credit_deducted: Boolean(d.credit_deducted)
        };
      } else {
        sessionData = session;
      }
  ...
  }, [currentSession, isTracking, checkAndDeductCredit]);
