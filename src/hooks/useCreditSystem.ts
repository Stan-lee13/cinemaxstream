...
export const useCreditSystem = () => {
  ...
  const getCreditLimits = (role: string, subscriptionExpiresAt?: string): CreditLimits => {
    // Check if premium subscription is still valid
    if (role === 'premium' || role === 'paid') {
  ...
  const canStream = (): boolean => {
    ...
    const limits = getCreditLimits(userProfile.role, userProfile.subscription_expires_at);
  ...
  return {
    ...
    getCreditLimits: () => userProfile ? getCreditLimits(userProfile.role, userProfile.subscription_expires_at) : null,
