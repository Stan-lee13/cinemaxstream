...
export const getContentDetails = async (id: string, type?: string): Promise<ContentItem | null> => {
  try {
...
export const getSimilarContent = async (id: string, type: string = 'movie'): Promise<ContentItem[]> => {
  try {
    const normalizedType = normalizeContentType(type);
