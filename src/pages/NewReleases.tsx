...
const NewReleases = () => {
  ...
  // Fetch latest releases from the past 30 days
  const { data: newMovies, isLoading: loadingMovies } = useQuery({
    queryKey: ['new-releases-movies'],
    queryFn: () => tmdbApi.getNewReleases(1),
  });

  const { data: newSeries, isLoading: loadingSeries } = useQuery({
    queryKey: ['new-releases-series'],
    queryFn: () => tmdbApi.getNewReleases(1),
  });
  ...
  const renderContentGrid = (content: ContentItem[], title: string) => (
    <div className="mb-12">
      <h2 className="text-2xl font-bold mb-6 flex items-center gap-2">
        <Calendar className="h-6 w-6 text-cinemax-500" />
        {title}
      </h2>
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="pt-24 pb-12">
        <div className="container mx-auto px-4">
          ...
          <div className="mb-8">
            <h1 className="text-3xl md:text-4xl font-bold mb-2">New Releases</h1>
            <p className="text-gray-400 max-w-2xl">
              Discover the latest movies and TV series released in the past 30 days. Stay up to date with the newest content!
            </p>
          </div>
          ...
          {isLoading ? (
            <>
              ...
              {renderSkeleton()}
              <h2 className="text-2xl font-bold mb-6">Latest TV Series</h2>
              {renderSkeleton()}
            </>
          ) : (
            <>
              {newMovies && newMovies.length > 0 && renderContentGrid(newMovies, "Latest Movies")}
              {newSeries && newSeries.length > 0 && renderContentGrid(newSeries, "Latest TV Series")}
              
              {(!newMovies || newMovies.length === 0) && (!newSeries || newSeries.length === 0) && (
                <div className="text-center py-12">
                  <Calendar className="h-16 w-16 text-gray-500 mx-auto mb-4" />
                  <h2 className="text-xl font-semibold mb-2">No new releases found</h2>
                  <p className="text-gray-400 mb-6">
                    Check back later for the latest content updates.
                  </p>
