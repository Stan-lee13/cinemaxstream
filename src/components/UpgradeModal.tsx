  ...
  const getMessage = () => {
    if (reason === 'streaming') {
      return currentRole === 'free'
        ...
        : "You've reached your daily streaming limit of 20 videos.";
    } else {
      return currentRole === 'free'
        ? "Downloads are not available on the free plan."
        : "You've reached your daily download limit of 10 files.";
    }
  };

  const handleUpgrade = async (planCode: string, amount: number) => {
    if (!user?.email) {
    ...
    try {
      ...
      await paystackService.openPaymentPopup({
        email: user.email,
        amount,
        plan: planCode,
        metadata: {
          ...
          {getUpgradeOptions().map((plan) => (
            ...
            >
              ...
              <Button
                ...
                onClick={() => handleUpgrade(plan.planCode, plan.amount)}
