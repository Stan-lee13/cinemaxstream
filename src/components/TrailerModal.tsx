    ...
    if (isOpen && trailerKey) {
      ...
      try {
        ...
        if (trailerKey.includes('youtube.com/watch?v=')) {
          ...
          videoId = trailerKey.split('youtu.be/')[1].split('/')[0];
        } else if (trailerKey.includes('embed/')) {
          videoId = trailerKey.split('embed/')[1].split('?')[0];
        }
        ...
        if (typeof window !== 'undefined') {
          ...
          if (globalWindow.errorReporter?.captureException) {
            void globalWindow.errorReporter.captureException(error as Error, 'TrailerModal', 'medium');
          }
        }
