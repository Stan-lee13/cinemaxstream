...
class CacheManager<T = unknown> {
  ...
  get<K = T>(key: string): K | null {
    ...
    return item.data as unknown as K;
  }
  ...
  getStats() {
    ...
    for (const [key, item] of this.cache.entries()) {
      ...
      try {
        ...
        // If data is not serializable, approximate
        totalSize += 0;
      }
    }
...
export const contentCache = new CacheManager<unknown>({
  maxSize: 200,
...
export const cacheContent = <T = unknown>(key: string, data: T, ttl?: number) => {
  contentCache.set(key, data, ttl);
};

export const getCachedContent = <T = unknown>(key: string): T | null => {
  return contentCache.get<T>(key);
};

export const cacheUser = <T = unknown>(key: string, data: T, ttl?: number) => {
  userCache.set(key, data, ttl);
};

export const getCachedUser = <T = unknown>(key: string): T | null => {
  return userCache.get<T>(key);
};

export const cacheSearch = <T = unknown>(key: string, data: T, ttl?: number) => {
  searchCache.set(key, data, ttl);
};

export const getCachedSearch = <T = unknown>(key: string): T | null => {
